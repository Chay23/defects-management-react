...
class Defects extends Component{
    ...
    handleDelete = async () => {
        const updatedDefectsList = this.state.defects.filter(defect => defect.id !== this.state.id);
        await axios.delete('/defects/' + this.state.id)
            .then(() => {
                this.setState({
    ...
    render() {
        const defectsList = this.state.defects.length !== 0 
        ? this.state.defects.map(defect => {
            ...
            return (
                <div key={defect.id} className={styles.defectBlock}>
                    <Link to={this.props.match.url+ '/' + defect.id}><p>{defect.title}</p></Link>
                    <p>{defect.description}</p>
                    <p>{defect.room}</p>
                    <p className={styles.status} style={statusColor}>{defect.info}</p>
                    <div className={styles.actionsBlock}>
                        <img src={trashIcon} onClick={() => this.handleConfirmationOpened(defect.id)} alt='Видалити'/>
                    </div>
                </div>
        )})
